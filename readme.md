...
## Git的相关使用
对于git工具的使用，我曾经写过一篇知乎文章介绍常用的一些git指令，[git学习笔记](https://zhuanlan.zhihu.com/p/526826127)
...
git feth & git rebase xxxx       // 用于合并分支
```

注意：在提交自己的代码到仓库当中时，最好以自己修改的内容为名称，根据自己commit的内容提交到一个全新的分支。



## 当遇到GEM5无法commit时，如何进行Debug

首先使用指令运行GEM5，运行前设定一个极大的tick，并等待其卡死
```
scripts-gem5/test-perl.sh DecoupleBP 100000000000 output_dir
```
接下来，使用指令判断是否出现卡死，并且找到lastCommitTick，如下图中所示，即发生卡死
```
killall gem5.opt -SIGUSR1 && sleep 2

grep "simTick\|committedInsts\|lastCommit" m5out/stats.txt
```

![lastCommitTick](img/lastCommitTick.jpg)

Ctrl+C停掉卡死的gem5，然后运行指令，可以得到一个log
```
scripts-gem5/test-perl.sh DecoupleBP 12448364500 output_dir
```
